    ...
    for name in name_list:
        ...
        if name == "TRACKINGNET":
            if settings.use_lmdb:
                ...
                datasets.append(
                    TrackingNet(settings.env.trackingnet_dir, image_loader=image_loader, env_num=settings.env_num))
    return datasets
...
def get_optimizer_scheduler(net, cfg):
    ...
    if cfg.TRAIN.OPTIMIZER == "ADAMW":
        ...
        optimizer = Adan(param_dicts,
                         lr=cfg.TRAIN.LR,
                         weight_decay=cfg.TRAIN.WEIGHT_DECAY,
                         max_grad_norm=cfg.TRAIN.MAX_GRAD_NORM)
    elif cfg.TRAIN.OPTIMIZER == 'Lion':
        optimizer = Lion(param_dicts,
                         lr=cfg.TRAIN.LR,
                         weight_decay=cfg.TRAIN.WEIGHT_DECAY)
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == 'step':
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
                                                                  T_max=cfg.TRAIN.SCHEDULER.COS_CYCLE,
                                                                  eta_min=0.0001)
    else:
